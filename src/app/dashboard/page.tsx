...
export default async function DashboardPage() {
  ...
  return (
    <PageLayout active="dashboard" username={session.user.username}>
      ...
      >
        <div>
          <h1 className="font-[Instrument_Serif] text-[28px] text-[#1a1a1a] leading-tight">
            Good to see you, <em className="italic text-[#999]">{session.user.username}.</em>
          </h1>
          <p className="text-[11px] text-[#bbb]"
          style={{"marginTop": "0.25rem"}}
          >Here's your interview prep overview</p>
        </div>
        <Link
          href="/analyze"
      ...
      style={{"marginBottom": "1.75rem"}}
      >
        <StatCard label="Total Analyses" value={totalReports.toString()} sub="All time" />
        <StatCard label="Avg Match Score" value={`${avgScore}%`} sub="Across all reports" />
        <StatCard label="Best Match" value={totalReports > 0 ? `${bestScore}%` : "—"} sub="Highest score" />
      </div>
...
function StatCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="bg-white border border-[#e8e6e1] rounded-xl"
    style={{ 
        "paddingInline":"1rem",
        "paddingBlock": "1rem",
      }}
    >
      <div className="text-[10px] text-[#aaa] uppercase tracking-[0.06em] mb-1"
